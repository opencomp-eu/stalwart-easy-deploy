...
1. Point DNS `A`/`AAAA` for the mail hostname (and webmail domain) at this server.
2. `bash apply.sh` (or `--unlock-proxy`) **completes Stalwart bootstrap** from `deploy.yaml`: hostname, domain, console logging, ACME off (Caddy already terminates HTTPS). You do not need the `/admin` wizard for first boot.
...
5. Publish MX / SPF / DKIM / DMARC. `python scripts/dns_records.py --config deploy.yaml` prints the records as JSON. Pass `--dkim-txt` once Stalwart has generated the key (Management → Domains → DNS zone file).
6. Sign in to Bulwark at `https://webmail.example.com` with a mailbox created in Stalwart, or with a Kanidm user when the engine wired LDAP identity. The Kanidm portal **Webmail** tile lands on that origin.
